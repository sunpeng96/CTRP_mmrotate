...
# @Time    : 09/05/2025
# @Author  : Peng Sun
# @FileName: ctrp_rotated_retinanet.py
# @Software: PyCharm

import torch
import torch.nn as nn
from .ctrp_utils import CTRPModule
...
class CTRPRotatedRetinaNet(RotatedBaseDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(CTRPRotatedRetinaNet, self).__init__(init_cfg)
        if pretrained:
            warnings.warn('DeprecationWarning: pretrained is deprecated, '
                          'please use "init_cfg" instead')
            backbone.pretrained = pretrained

        self.use_ctrp = use_ctrp

        self.ctrp = CTRPModule(bbox_head_cfg=bbox_head)
    ...
    def forward_train(self,
        ...
        for i in range(self.num_refine_stages):
            ...
            reasoning_outs = []
            if self.use_ctrp:
                reasoning_feats = self.ctrp(x, outs[0], self.bbox_head, img_metas)
                for x_refine_scale, reasoning_feat in zip(x_refine, reasoning_feats):
                    reasoning_feature = torch.cat((x_refine_scale, reasoning_feat), dim=1)
                    reasoning_feature = self.conv1(reasoning_feature)
                    reasoning_outs.append(reasoning_feature)

            outs = self.refine_head[i](reasoning_outs)
            loss_inputs = outs + (gt_bboxes, gt_labels, img_metas)
    ...
    def simple_test(self, img, img_meta, rescale=False):
        ...
        for i in range(self.num_refine_stages):
            ...
            reasoning_outs = []
            if self.use_ctrp:
                reasoning_feats = self.ctrp(x, outs[0], self.bbox_head, img_meta)
                for x_refine_scale, reasoning_feat in zip(x_refine, reasoning_feats):
                    reasoning_feature = torch.cat((x_refine_scale, reasoning_feat), dim=1)
                    reasoning_feature = self.conv1(reasoning_feature)
                    reasoning_outs.append(reasoning_feature)
