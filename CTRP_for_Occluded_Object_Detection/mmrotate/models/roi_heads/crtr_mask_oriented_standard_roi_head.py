...
# @Time    : 11/02/2023
# @Author  : Peng Sun
# @FileName: crtr_mask_oriented_standard_roi_head.py
# @Software: PyCharm
...
class CRTrMaskOrientedStandardRoIHead(OrientedStandardRoIHead):
    """
    Oriented RCNN roi head including one bbox head with 'SpatialReasoningTransformer'
    """
    def __init__(self,
        ...
        super(CRTrMaskOrientedStandardRoIHead, self).__init__(bbox_roi_extractor,
    ...
    def simple_test_mask_reason(self, x, img_metas,
        ...
        for pos_bbox, pos_label, mask_proposal, img_meta in zip(pos_bboxes, pos_labels, mask_proposals, img_metas):
            ...
            if pos_bbox.numel() != 0 and mask_proposal.numel() != 0:
                # 1. 构造 Relation rois
                rel_regions_list, rel_pairs_list, sub_labels_list, spatial_embeddings_list = \
                    self.mask_reason_head.rel_pair_sampler([pos_bbox[:, :5]], [pos_label], [mask_proposal[:, :5]], [img_meta], topK=self.topK)
                # rel_pair_bboxes (List[Tensor]): Union region of subject_bbox and mask_bbox
                # subject_labels (Tensor): Subject label corresponding to rel_pair_bboxes
