from fair_to_dota import fair_to_dota
...
def add_parser(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='images annotation dirs, must give a value')
    ...
    parser.add_argument(
        ...
        type=str,
        default='.',
        help='to save converted dota format images')
    parser.add_argument(
        '--save-ext',
        type=str,
        default='.png',
...
def parse_args():
    ...
    # assert arguments
    print(args)
    assert args.img_ann_dirs is not None, "argument img_ann_dirs can't be None"
...
def main():
    ...
    args = parse_args()
    print("=====Converting FAIR dataset to DOTA format!=====")

    # fair_to_dota(args.img_ann_dirs, args.converted_dota_dir)
    ann_dir = os.path.join(args.converted_dota_dir,"labelTxt")
    img_dir = os.path.join(args.converted_dota_dir,"images")
    ...
    if args.nproc > 1:
        ...
        patch_infos = pool.map(worker, zip(infos, img_dirs_))
        pool.close()
    else:
        patch_infos = list(map(worker, zip(infos, img_dirs_)))
