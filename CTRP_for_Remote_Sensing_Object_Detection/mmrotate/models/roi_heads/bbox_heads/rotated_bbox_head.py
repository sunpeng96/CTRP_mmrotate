...
class RotatedBBoxHead(BaseModule):
    ...
    def get_bboxes(self,
        ...
        if cfg is None:
            ...
            if synth_cfg is None:
                ...
                filtered_bboxes, keep_scores, keep_labels = multiclass_preprocess(bboxes, scores, cfg.score_thr)
                # filtered_bboxes (torch.Tensor): shape(N, 5) [x, y, w, h, theta]
                # keep_scores (torch.Tensor): shape(n, score) or (n, 5)
                # keep_labels (torch.Tensor): shape(N)
                det_bboxes, det_labels = synth_rotated(filtered_bboxes, keep_scores, keep_labels, cfg.deepcopy())
    ...
    @force_fp32(apply_to=('bbox_preds', ))
    def refine_rboxes(self, rois, labels, bbox_preds, pos_is_gts, img_metas):
        """Refine bboxes during training.

        Args:
            rois (torch.Tensor): Shape (n*bs, 6), where n is image number per
                GPU, and bs is the sampled RoIs per image. The first column is
                the image id and the next 5 columns are x,y,w,h,a.
            labels (torch.Tensor): Shape (n*bs, ).
            bbox_preds (torch.Tensor): Shape (n*bs, 5) or (n*bs, 5*#class).
            pos_is_gts (list[Tensor]): Flags indicating if each positive bbox
                is a gt bbox.
    ...
    @force_fp32(apply_to=('bbox_preds', ))
    def refine_rboxes_hbb(self, rois, labels, bbox_preds, pos_is_gts, img_metas):
        """Refine bboxes during training.
        ...
        for i, _ in enumerate(img_metas):
            ...
            bboxes = self.regress_by_class_hbb(bboxes_, label_, bbox_pred_,
                                           img_meta_)
    ...
    @force_fp32(apply_to=('bbox_pred', ))
    def regress_by_class_hbb(self, rois, label, bbox_pred, img_meta):
        """Regress the bbox for the predicted class. Used in Cascade R-CNN.
        Args:
