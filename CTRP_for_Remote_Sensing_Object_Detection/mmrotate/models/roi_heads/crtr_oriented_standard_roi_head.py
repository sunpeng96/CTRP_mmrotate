...
from mmrotate.core import rbbox2roi, build_assigner, build_sampler, rbbox2result, build_bbox_coder
...
class CRTrOrientedStandardRoIHead(RotatedStandardRoIHead):
    """
    ...
    def __init__(self,
                 ...
                 use_DRL=False,
                 temperture=3,
                 adj_gt='mmrotate/models/roi_heads/reasoning_model/dota_graph_r.pkl',
        ...
        super(CRTrOrientedStandardRoIHead, self).__init__(init_cfg)
        ...
        # init cmp attention
        if self.use_feat_enhance is True:
        ...
        if init_bbox_head and refined_bbox_head is not None:
            self.init_bbox_head(bbox_roi_extractor, init_bbox_head, refined_bbox_head)
        ...
        self.roi_feat_size = bbox_roi_extractor.roi_layer.out_size

        in_channels = 256
        if shared_num_fc > 0:
            in_channels *= (self.roi_feat_size * self.roi_feat_size)

        assert init_bbox_head.num_classes == refined_bbox_head.num_classes
    ...
    def init_bbox_head(self, bbox_roi_extractor, init_bbox_head=None, refined_bbox_head=None):
        ...
        assert len(bbox_roi_extractor) == self.num_stages
    ...
    def init_assigner_sampler(self):
        ...
        if self.train_cfg is not None:
            ...
            refined_bbox_assigner = build_assigner(self.train_cfg.refined_rcnn.assigner)

            init_bbox_sampler = build_sampler(self.train_cfg.init_rcnn.sampler, context=self)
            refined_bbox_sampler = build_sampler(self.train_cfg.refined_rcnn.sampler, context=self)

            self.bbox_assigner = [init_bbox_assigner, refined_bbox_assigner]
            self.bbox_sampler = [init_bbox_sampler, refined_bbox_sampler]
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if self.use_feat_enhance and i > 0:
                ...
                # 2.compute reasoning_model attention
            ...
            if self.current_stage < self.num_stages - 1:
                ...
                with torch.no_grad():
                    proposal_list = self.bbox_head[self.current_stage].refine_rboxes(
    ...
    def simple_test_bboxes(self,
        ...
        for i in range(self.num_stages):
            ...
            if self.current_stage < self.num_stages - 1:
                ...
                rois = self.bbox_head[self.current_stage].regress_by_class(rois, bbox_label, bbox_pred,
    ...
    def forward_dummy(self, x, proposals, num_imgs=None):
        ...
        for i in range(self.num_stages):
            ...
            if self.current_stage < self.num_stages - 1:
                ...
                # Produce tensor for reasoning model
                bbox_pred_decode = self.bbox_coder.decode(rois, bbox_pred, (1024, 1024, 3))
                bbox_feats = self.bbox_roi_extractor[self.current_stage](
                    x[:len(self.bbox_roi_extractor[self.current_stage].featmap_strides)],
                    rois)  # shape (1024, 256, 7, 7)
