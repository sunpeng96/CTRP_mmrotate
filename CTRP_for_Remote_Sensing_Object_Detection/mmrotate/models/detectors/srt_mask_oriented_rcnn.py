...
from .srt_utils import mask_detector, cloud_detector
from .img_random_mask import random_mask

@ROTATED_DETECTORS.register_module()
class SrtMaskOrientedRCNN(RotatedTwoStageDetector):
    """
    Implementation 'Spatial Reasoning Transformer' on 'Oriented R-CNN'
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SrtMaskOrientedRCNN, self).__init__(
            backbone=backbone,
            neck=neck,
            rpn_head=rpn_head,
            roi_head=roi_head,
    ...
    def forward_train(self,
        ...
        if self.use_mask_img:
            rand_mask_results, gt_bboxes, gt_labels = random_mask(img, img_metas, self.mask_ratio,
                                                                     gt_bboxes, gt_labels)
            # rand_mask_results (dict): List of mask_result where each dict
            #   has: 'mask_img', 'gt_bboxes', 'gt_labels', 'gt_mask_bboxes', 'gt_mask_labels'
            # gt_bboxes, gt_labels (List[Tensor]): Gt bboxes or labels without masked objects
        ...
        if self.use_mask_img and self.roi_head_type == 'SrtMaskOrientedStandardRoIHead':
            ...
            for i in range(len(image)):
                mask_proposal = cloud_detector(image[i])  # 'mask_detector' or 'cloud_detector'
            ...
            roi_losses = self.roi_head.forward_train(x, img_metas, proposal_list,
                                                     ...
                                                     gt_bboxes_ignore, gt_masks,
                                                     **kwargs)
        # 检查损失函数反向回传的数值, 判断参数冻结是否成功.
        # for name, p in self.roi_head.named_parameters():
        #     if 'binary_classifier.bias' in name:
        #         print(11111, name, 22222, p.requires_grad)
        #         print(33333, p)
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        for i in range(len(image)):
            ...
            mask_proposal_np = cloud_detector(image[i])  # 'mask_detector' or 'cloud_detector'
