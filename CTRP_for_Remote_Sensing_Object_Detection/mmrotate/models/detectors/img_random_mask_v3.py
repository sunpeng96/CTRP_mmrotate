...
# @Time    : 02/21/2024
# @Author  : Peng Sun
# @FileName: img_random_mask_v3.py
# @Software: PyCharm
...
def random_masking(img, img_metas,
    ...
    for i, (img_batch, gt_bboxes, gt_labels, img_meta) in enumerate(zip(img, gt_bboxes_list, gt_labels_list, img_metas)):
        ...
        if num_gt == 1:  # 当图像中只有一个 GT时, 不对图像作掩膜处理;
            ...
            alpha = 0.95
            color = (255, 255, 255)
            mask = np.zeros([img_shape[0], img_shape[1], 3], dtype=np.uint8)  # 构造一个与输入图像尺寸一致的掩膜图像
            ...
            img_batch_np = img_batch.reshape(img_shape[0], img_shape[1], -1).detach().cpu().numpy()
            mask_img = cv2.addWeighted(img_batch_np, 1.0, mask, alpha, 1, dtype=cv2.CV_32F)
            new_img_batch = torch.tensor(mask_img, device=img_device).reshape(-1, img_shape[0], img_shape[1])
